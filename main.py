...
def get_langsmith_client() -> Any:
    ...
    api_key = safe_text(LANGCHAIN_API_KEY or LANGSMITH_API_KEY)
    if not api_key:
        raise RuntimeError("Missing LANGCHAIN_API_KEY or LANGSMITH_API_KEY.")

    kwargs: Dict[str, Any] = {"api_key": api_key}
    endpoint = safe_text(LANGCHAIN_ENDPOINT)
    if endpoint:
...
def render_langsmith_tracing_tab() -> None:
    ...
    project_name = controls[0].text_input("Project", value=LANGCHAIN_PROJECT, key="ls_project_main")
